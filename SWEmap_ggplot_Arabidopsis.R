...
## download Sweden shapefile data

sweden<-getData("GADM",country="SE",level=0)

## make base map

swe<-ggplot()+geom_polygon(data=sweden,aes(x=long,y=lat,group=group))+coord_fixed(1.3)
...
## obtaining the SWE specific accessions from dataset

SWEdata<-ecotypes_all[which(ecotypes_all$country.x=="SWE"),]

## cast ecotype locations as lat long coordinates on base map

swe+theme_dark() +
  geom_point(data=SWEdata,aes(x=long,y=lat,color=factor(genotype_comp)),size=1.7,alpha=0.7) +
...
ggsave('Sweden_map_LHY_haplotype.png')
